...
class TestFormatRow:
    def test_short_summary_single_line(self):
        ...
        lines = format_row("[indexed]", "sess-abc-123", "Short summary", 40)
        assert len(lines) == 1
        assert "[indexed]" in lines[0]
    ...
    def test_long_summary_wraps(self):
        ...
        lines = format_row("[indexed]", "sess-abc-123", long_summary, 30)
        assert len(lines) > 1

    def test_continuation_indent(self):
        """Continuation lines are indented to the summary column."""
        long_summary = "word " * 40
        lines = format_row("[indexed]", "sess-abc-123", long_summary, 20)
        assert len(lines) > 1
    ...
    def test_empty_summary(self):
        ...
        lines = format_row("[unindexed]", "sess-xyz-789", "", 40)
        assert len(lines) == 1
        assert "[unindexed]" in lines[0]
...
class TestPrintPlain:
    def test_prints_header_and_rows(self, capsys):
        ...
        rows = [
            {"status": "[indexed]", "session_id": "sess-001", "summary": "First"},
            {"status": "[unindexed]", "session_id": "sess-002", "summary": "Second"},
        ]
        _print_plain(rows)
