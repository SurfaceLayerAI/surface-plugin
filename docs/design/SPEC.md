...
Claude Code records full session transcripts including the agent's chain-of-thought reasoning (thinking blocks), tool calls, and user interactions. When a developer finishes their implementation, they run `/surface:browse` to see a Haiku-summarized index of recent sessions and interactively select from the most recent plan-mode sessions. The plugin then extracts key decisions and tradeoffs from the session transcripts and produces a structured PR description.
...
surface ships as a Claude Code plugin directory:
...
`plugin.json` declares the plugin name, version, description, and registered hooks. The `skills/` directory contains command definitions: each subdirectory maps to a `/surface:<command>` skill. The `hooks/` directory registers lifecycle hooks (e.g., `SessionEnd` for indexing). The `scripts/` directory holds standalone Python scripts invoked by hooks and commands.

Installation: run `/plugin install <path-or-url>` from Claude Code, or launch Claude Code with `claude --plugin-dir ./surface`. Both methods register the plugin for the current project. Command namespacing (`/surface:<command>`) prevents collisions with other plugins or built-in commands.
...
**Phase 1: Extraction** (on-demand via `/surface:describe`):
...
Signals are written to an intermediate JSONL file (`.surface/<session>.signals.jsonl`) with a typed schema designed for extensibility: future signal types can be added without changing existing processing.
...
**Phase 2: Synthesis** (on-demand via `/surface:describe` command):
...
Before synthesis, `/surface:describe` estimates the combined token count of all selected signals files. If the estimate exceeds 100K tokens (roughly 50% of the context window, the general threshold for performance degradation), it warns the developer and recommends selecting fewer sessions or a narrower scope. Automatic chunking/prioritization is out of scope for v0.

#### Multi-session synthesis

When `/surface:describe` receives multiple session IDs, synthesis narrates the full reasoning arc across sessions. The output is biased toward the final session's state as authoritative, since later sessions reflect the most current decisions. The reviewer sees how the approach evolved: initial direction, mid-course corrections, and the rationale behind the final state.

### Session indexing

When a Claude Code session ends, a `SessionEnd` hook runs a lightweight indexing step. A Python script reads the transcript, extracts key structural elements (session ID, timestamp, initial user request, plan file paths, files touched), and passes these snippets to Haiku with a prompt to produce a 2-3 sentence summary. The result is appended to `.surface/session-index.jsonl`:

```jsonl
{"session_id": "...", "timestamp": "...", "summary": "...", "plan_mode": true, "plan_paths": ["plans/auth-redesign.md"]}
```

The index lets the developer scan their sessions and choose which ones to feed into `/surface:describe`. A session ID and timestamp alone are not enough: the developer needs to understand *what happened*. Haiku turns structural metadata into a human-scannable summary at negligible cost (~$0.25/MTok input; a few thousand tokens of extracted snippets costs fractions of a cent per session).

### Hooks and triggers

| Hook | Event | Purpose |
|---|---|---|
| `SessionEnd` | Claude Code session ends | Runs lightweight indexing: extracts metadata from transcript, calls Haiku to produce summary, appends to `.surface/session-index.jsonl` |

| Command | Trigger | Purpose |
|---|---|---|
| `/surface:browse` | Developer runs manually | Displays the session index, then presents up to 4 most recent plan-mode sessions as AskUserQuestion options. On selection, chains into extraction (Phase 1) and synthesis (Phase 2). "Other" accepts manual session IDs. |
| `/surface:describe <session-id> [session-id...]` | Developer runs manually with explicit session IDs | Standalone entry point for extraction (Phase 1) and synthesis (Phase 2) on specified sessions, outputs PR description |

AskUserQuestion supports 2–4 options. `/surface:browse` shows the 4 most recent plan-mode sessions. Sessions beyond that require manual IDs via "Other" or `/surface:describe` directly.
...
**Why SessionEnd and not automatic extraction:** Development is non-linear. A developer may work across many sessions, revisit earlier work, or abandon threads. Running full extraction on every SessionEnd wastes work on sessions that may never be relevant to the final PR. The developer knows which sessions matter and selects them explicitly via `/surface:describe`.

**SessionEnd firing conditions:** The hook receives `session_id` and `transcript_path`. Known reasons for firing: `clear`, `logout`, `prompt_input_exit`, `bypass_permissions_disabled`, `other`. If SessionEnd does not fire (e.g., process killed), the session transcript still exists on disk. `/surface:describe` can process any session by ID regardless of whether SessionEnd indexed it. The index is a convenience, not a requirement.
...
- On-demand extraction and synthesis via `/surface:describe` with explicit session selection
- Multi-session support: `/surface:describe` accepts multiple session IDs for PRs spanning several sessions
...
3. Developer runs `/surface:browse`, reviews the displayed session index, and selects sessions from the interactive prompt (or uses `/surface:describe` directly with explicit session IDs).
4. Plugin extracts signals, estimates context size (warns if >100K tokens), runs synthesis, and outputs a structured PR description.
5. Plugin displays the generated PR description for developer review.
6. On confirmation, the plugin creates the PR via `gh pr create --body`.
7. Reviewer reads the PR description and starts reviewing. No meeting needed.
...
|---|---|---|
| Transcript missing (process killed) | Yes | `/surface:describe` reads from disk by session ID. If SessionEnd did not fire, the index lacks the entry, but the transcript file persists. The developer provides the session ID directly. |
